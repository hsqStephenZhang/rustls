        ...
        Ok(match self {
            ...
            CraftExtension::KeyShare(key_share_spec) => {
                if hrr.is_some()
                    && hrr
                        .unwrap()
                        .requested_key_share_group()
                        .is_some()
                    || !craft_config.override_keyshare
                {
                    return ext_store
                        .remove(&ExtensionType::KeyShare.into())
                        .ok_or(());
                }
                let mut origin_ks = get_origin_ext!(
                    ext_store.remove(&ExtensionType::KeyShare.into()),
            ...
            CraftExtension::Padding => {
                ClientExtension::CraftPadding(CraftPadding {
                    psk_len: if let Some(ClientExtension::PresharedKey(psk)) =
                        ext_store.get(&ExtensionType::PreSharedKey.into())
                    {
...
impl Codec<'_> for CraftPadding {
    ...
    fn read_bytes(bytes: &'_ [u8]) -> Result<Self, crate::InvalidMessage> {
        let mut reader = crate::msgs::codec::Reader::init(bytes);
    ...
    fn patch_extension(
        ...
        for ext in extension.drain(..) {
            match ext {
                ...
                | ClientExtension::Cookie(_) => {
                    ext_store.insert(ext.ext_type().into(), ext);
                }
                ClientExtension::CertificateCompressionAlgorithms(_) => {
                    ext_store.insert(ext.ext_type().into(), ext);
                }
                _ => (),
            }
        ...
        for idx in order {
            ...
            extension.push(match spec {
                ...
                Keep(Must(ext_type)) => match ext_store.remove(&(*ext_type).into()) {
                    Some(ext) => ext,
                    None => {
                ...
                Keep(Optional(ext)) => match ext_store.remove(&(*ext).into()) {
                    Some(ext) => ext,
                    None => {
                        continue;
                    }
